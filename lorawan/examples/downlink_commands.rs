...
static mut UPLINK_INTERVAL_MS: u32 = 60_000;
...
fn main() -> ! {
    ...
    // Main loop - send periodic uplinks and handle downlink commands
            ...
            if let Ok(Some(downlink)) = device.process() {
                ...
                match downlink {
                    DownlinkCommand::SetInterval(interval_seconds) => {
                        ...
                        unsafe {
                            UPLINK_INTERVAL_MS = interval_seconds as u32 * 1000;
                        }
                    }
                    DownlinkCommand::ShowFirmwareVersion => {
                        // Send firmware version in next uplink
                        let version = b"1.0.0";
                        device.send_uplink(224, version, false).ok();
                    }
                    ...
                    DownlinkCommand::Custom(port, payload) => {
                        // Handle custom command (echo back payload)
                        device.send_uplink(port, &payload, false).ok();
                    }
                }
            }
        }

        // Wait for configured interval before next transmission
        unsafe {
            delay.delay_ms(UPLINK_INTERVAL_MS);
        }
